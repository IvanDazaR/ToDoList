...
require_once 'Task.php'; // usamos PSR4 para nuestra clase task
...
$map->get('todo.list', '/', function ($request) use ($twig) {
    ...
    //     ]
    // ];
    $tasks = Task::all(); // nostrae todas los elementos para las consultas con Laravel
...
$map->post('todo.add', '/add', function ($request) {
    $data = $request->getParsedBody(); // Nos regresa un arreglo con los elementos del formulario
    $task =  new Task(); 
...
$map->get('todo.check', '/check/{id}', function ($request) {
    $id = $request->getAttribute('id'); // Nos regresa un arreglo con los elementos del formulario
    $task =  Task::find($id); 
    $task->done = true;
    $task->save();
    $response = new Zend\Diactoros\Response\RedirectResponse('/');
